    ...
    <section className="flex flex-col md:flex-row items-center justify-center min-h-[90vh] gap-8 px-6 py-12">
      <div className="md:w-1/2 space-y-8">
        <div className="text-center md:text-left">
          ...
          <AnimatePresence>
            {taglineVisible && (
              <motion.h2
                ...
                className={`text-2xl md:text-3xl mb-6 font-['Poppins'] ${darkMode ? 'text-gray-200' : 'text-gray-700'  // Better contrast for light mode
                  }`}
              >
                <span className="relative inline-block">
                  <span className={darkMode ? 'text-teal-500' : 'text-teal-600'}>  {/* Darker teal for light mode */}
                    {tagline.split('').map((char, i) => (
                      <motion.span
                        key={i}
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0, transition: { delay: 0.5 + (i * 0.03), type: "spring", stiffness: 300 } }}
                      >
                        {char}
                      </motion.span>
                    ))}
                  ...
                  <motion.span
                    initial={{ scaleX: 0 }}
                    animate={{ scaleX: 1, transition: { delay: 0.5 + (tagline.length * 0.03) + 0.3, duration: 0.5, ease: "easeOut" } }}
                    className={`absolute bottom-0 left-0 w-full h-1 transform origin-left ${darkMode ? 'bg-blue-400' : 'bg-blue-500'  // Darker blue for light mode
                      }`}
                  />
                </span>
              </motion.h2>
          ...
          <motion.p
            ...
            className={`text-lg md:text-xl mb-8 leading-relaxed font-medium ${  // Added font-medium for better readability
              darkMode ? 'text-gray-300' : 'text-gray-800'  // Much darker text for light mode
              }`}
          >
          ...
          {buttonsVisible && (
            ...
            >
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                href="#projects"
                className={`px-6 py-3 rounded-lg font-semibold flex items-center text-white transition-all shadow-lg ${darkMode
              ...
              <motion.a
                ...
                href="/resume.pdf"
                download
                className={`px-6 py-3 rounded-lg font-semibold flex items-center gap-2 transition-all shadow-lg ${darkMode
                    ? 'bg-gray-700 hover:bg-gray-600 text-white shadow-gray-700/25'
                    : 'bg-gray-800 hover:bg-gray-700 text-white shadow-gray-800/25'  // Darker background for better contrast
                  }`}
              >
                <FaFileDownload />
                Download CV
          ...
          {[
            ...
            <motion.a
              ...
              className={`p-3 rounded-full transition-colors shadow-lg ${darkMode
                  ...
                  : 'bg-gray-100 text-gray-800 hover:bg-gray-200 shadow-gray-500/20 border border-gray-300'  // Added border and darker text
                }`}
            >
              {social.icon}
            </motion.a>
      ...
      >
        <div className={`relative w-64 h-64 md:w-80 md:h-80 rounded-full overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 group home-img ${darkMode ? 'ring-4 ring-teal-500' : 'ring-4 ring-blue-600'
          ...
          />
          <motion.div
            className={`absolute -bottom-4 -right-4 p-2 rounded-full shadow-lg ${darkMode ? 'bg-gray-800' : 'bg-white border border-gray-200'  // Added border for light mode
              }`}
            initial={{ scale: 0 }}
